...
move_downloaded_file <- function(local_folder, local_file)
{
    downloaded_file <- normalizePath(file.path(getwd(),local_folder, local_file))
    if (file.exists(downloaded_file)) {
      target_path <- normalizePath(file.path(local_folder, local_file), mustWork = FALSE)
      
      if (!dir.exists(dirname(target_path))) {
        dir.create(dirname(target_path), recursive = TRUE)
      }
      file.copy(downloaded_file, target_path)
...
get_run_config <- function(configure_run_file = "configure_run.yml", lake_directory, config, clean_start = FALSE, config_set_name = "default", sim_name = NA){
  ...
  if(!config$run_config$use_s3 | clean_start){
    ...
    restart_exists <- tryCatch({
     FaaSr::faasr_get_file(server_name = server_name,remote_folder = remote_folder, remote_file = remote_file, local_folder = local_folder, local_file = local_file)
      downloaded_file <- normalizePath(file.path(getwd(),local_folder, local_file))
      #downloaded_file <- normalizePath(local_folder, local_file)
      if (file.exists(downloaded_file)) {
        TRUE
      } else {
      ...
      print("Error in getting restart file")
      FALSE
    })
    ...
    if(restart_exists){
      ...
      file.copy(downloaded_file, target_path,overwrite = TRUE)
      file.remove(downloaded_file)
      ...
      #message("in else")
      message("restart does not exist")
      yaml::write_yaml(run_config, file.path(lake_directory,"restart", config$location$site_id, sim_name, configure_run_file))
    }
  }
...
put_targets <- function(site_id, cleaned_insitu_file = NA, cleaned_met_file = NA, cleaned_inflow_file = NA, use_s3 = FALSE, config){
  ...
  if(use_s3){
    if(!is.na(cleaned_insitu_file)){
      ...
      message("Reached faasr call for put targetssssssssssssssssssss")
...
get_targets <- function(lake_directory, config){
  if(config$run_config$use_s3){
    ...
    .faasr <- config$faasr
    message("Reached faasr call for get targets download s3 objectssssssssss")
...
get_restart_file <- function(config, lake_directory){
  ...
  if(!is.na(config$run_config$restart_file)){
    ...
    if(config$run_config$use_s3){
      ...
      FaaSr::faasr_get_file(server_name = config$s3$restart$bucket, 
                            remote_folder = remote_folder,           
                            remote_file = remote_file,                
                            local_folder = local_folder,      
                            local_file = local_file)
      
      move_downloaded_file(local_folder,local_file)
...
update_run_config <- function(lake_directory,
  ...
  message("reached updateeeeeeee run config")
  ...
  if(use_s3){
    ...
    FaaSr::faasr_put_file(server_name = bucket,
                   local_folder = local_folder,
                   local_file = local_file,
                   remote_folder = remote_folder,
                   remote_file = remote_file)
...
download_s3_objects <- function(lake_directory, bucket, prefix, region, base_url, faasr_config){
  ...
  files <- FaaSr::faasr_get_folder_list(server_name = server_name,prefix=prefix)
...
delete_restart <- function(site_id, sim_name, bucket = "restart", endpoint,config){
  ...
  message("reached delete restart filesssssssss")
  ...
  files <- FaaSr::faasr_get_folder_list(server_name = server_name, prefix = prefix_path)
  keys <- vapply(files, `[[`, "", "Key", USE.NAMES = FALSE)
  empty <- grepl("/$", keys)
  keys <- keys[!empty]
  if(length(keys > 0)){
...
check_noaa_present <- function(lake_directory, configure_run_file = "configure_run.yml", config_set_name = "default"){
  ...
  message("reached check noaaaaaaaaaaa")
...
delete_sim <- function(site_id, sim_name, config){
  ...
  message("reached delete simmmmmmmmmmmm")
